...
#ifdef LEGACY_ACTIVATION_SHIM_ALLOW_LEGACY_API_FALLBACK
...
// Define shims for functions declared in mscoree.h only when the header has been included.
#ifdef __mscoree_h__
        ...
        __out DWORD  *pdwLength)
    {
        ...
        CALL_LEGACY_API(GetRequestedRuntimeInfo,
                        ...
                        (pExe,
                         ...
                         pConfigurationFile,
                         startupFlags,
                         (runtimeInfoFlags | 0x80 /*RUNTIME_INFO_CONSIDER_POST_2_0*/),
                         pDirectory,
                         dwDirectory,
                         pdwDirectoryLength, 
                         pVersion,
        ...
        int exitCode)
    {
        ...
        if (Util::HasNewActivationAPIs())
        {
		    ...
		    IfHrFailRetVOID(Util::GetCLRMetaHost(&pMH));
            IfHrFailRetVOID(pMH->ExitProcess(exitCode));
        }
        else
        {
			CALL_LEGACY_API_VOIDRET(CorExitProcess,
							(int exitCode),
							(exitCode));
		}
    }
...
// These methods are declared in cor.h, so define only if _COR_H_ has been included
#ifdef _COR_H_
    // --------------------------------------------------------------------------------------------
    inline 
...
// StrongName functions are declared in strongname.h
...
#define LEGACY_STRONGNAME_API_PASS_THROUGH_VOIDRET(_name, _sig, _args)   \
    LEGACY_API_PASS_THROUGH_STATIC_VOIDRET(_name, _sig, _args)
#else // defined(LEGACY_ACTIVATION_SHIM_DELAY_LOAD)
#define LEGACY_STRONGNAME_API_PASS_THROUGH(_name, _ret_type, _ret_value, _sig, _args)   \
...
#undef GET_CLRMETAHOSTPOLICY
#undef GET_CLRRUNTIMEINFO
#undef GET_CLRMETAHOST
...
// Restore all modified warning settings.
#pragma warning(pop)
