...
// ---LEGACY_ACTIVATION_SHIM_LOAD_LIBRARY----------------------------------------------------------
// Allow users of these headers to provide custom 'LoadLibrary' implementation.
#pragma push_macro("LEGACY_ACTIVATION_SHIM_LOAD_LIBRARY")
...
namespace LegacyActivationShim
{
    ...
    namespace Util
    {
        ...
        protected:
            // Note that this will not work if TYPE has non-default alignment requirements.
        ...
        // initialized data in the image. This will fail  for other scenarios, as the memory
        // used for the object may not be zero-initialized, which will result in incorrect
        // behaviour.
        template <typename TYPE, void (*ASSIGNF)(TYPE &), void (*RELEASEF)(TYPE &)>
        ...
        public:
            // Will LoadLibrary wzDllName and FreeLibrary on destruct.
            ...
            // Does not release hMod on destruct.
		...
		// T must be a strict function typedef, not a function pointer typedef. This will produce
		// a build error to prevent inadvertent misuse. See uses below for examples.
        template <typename T>
        class DelayLoadFunctor<T *>;

        // ---ZEROINITGLOBALSPINLOCK----------------------------------------------------------------
        // This does not use the standard Windows synchronization functions because that would
        // require heap allocation of a CRITICAL_SECTION object, and these headers are free of
        // heap allocations. Therefore, use this class only in low-frequency, low-contention
        // scenarios.
        ...
        public:
            typedef LegacyActivationShim::Util::Holder<
                     ZeroInitGlobalSpinLock*,
            ...
            LPVOID  *ppInterface)
        {
            ...
            MscoreeFunctor<CLRCreateInstance_pfn> pfnCLRCreateInstance;
            IfHrFailRet(pfnCLRCreateInstance.Init("CLRCreateInstance"));
            ...
            /*out*/ ICLRMetaHostPolicy **ppICLRMetaHostPolicy)
        {
            ...
            if (g_pCLRMetaHostPolicy == NULL)
            {
                ...
                IfHrFailRet(CallCLRCreateInstance(
        ...
        HRESULT GetCLRRuntimeInfoHelper(
            ...
            __out_ecount_opt(*pcchVersion) LPWSTR wszVersion = NULL,
            ...
            DWORD   *pcchImageVersion = NULL)
        {
            ...
            {   //
                ...
                hr = pMetaHostPolicy->GetRequestedRuntime(
                    static_cast<METAHOST_POLICY_FLAGS>(METAHOST_POLICY_HIGHCOMPAT |
                                                       METAHOST_POLICY_USE_PROCESS_IMAGE_PATH |
            ...
            {   //
                ...
                typedef HRESULT __stdcall GetRequestedRuntimeInfo_pfn(
                    LPCWSTR pExe,
                    LPCWSTR pwszVersion,
                ...
                DelayLoadFunctor<GetRequestedRuntimeInfo_pfn> pfnGetRequestedRuntimeInfo;
                IfHrFailRet(pfnGetRequestedRuntimeInfo.Init(hMscoree, "GetRequestedRuntimeInfo"));
                ...
                IfHrFailRet(pfnGetRequestedRuntimeInfo()(
                    NULL,
                    NULL,
                    NULL,
                    0,
            ...
            DWORD *pcchBuffer)
        {
            ...
            *pcchBuffer = pRuntimeInfo->m_cchImageVersion;
            ...
            if (cchBuffer <= pRuntimeInfo->m_cchImageVersion && wzBuffer != NULL)
            {
                wcsncpy_s(
                    wzBuffer,
                    cchBuffer,
        ...
        // ---AddStartupFlags------------------------------------------------------------------------------
		// Combines dwStartupFlags with those already set, and overwrites any host file that may have been
		// set.
        inline
            ...
            LPCWSTR wszHostConfigFile)
        {
			...
			// have been set explicitly, or have been set based on a config file. We want
			// to clear the concurrent GC flag because we are supplying non-defaults, and
			// combine them with the user supplied flags. Note that STARTUP_CONCURRENT_GC
			// is never set as part of reading a config so we are not losing any information
			// here.
            ...
            IStream **ppIStream)
        {
			...
			*ppIStream = NULL;
			...
			DelayLoadFunctor<SHCreateStreamOnFile_pfn> pfnCreateStreamOnFile;
			IfHrFailRet(pfnCreateStreamOnFile.Init(g_hShlwapi, "SHCreateStreamOnFileW"));

            // Create IStream
            IStream* pStream(NULL);
            IfHrFailRet(pfnCreateStreamOnFile()(wszFilePath, 0 /*STGM_READ*/, ppIStream));
...
#pragma pop_macro("LEGACY_ACTIVATION_SHIM_LOAD_LIBRARY")
